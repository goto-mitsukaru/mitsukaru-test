...
class GoogleSheetRecruitController extends Controller
{
    ...
    public function webhook(Request $request)
    {
        ...
        foreach ($sheet_data as $index => $row) {
            ...
            $name = data_get($row, 0, '');
            $scale_id = data_get($row, 1, '');
            $status_id = data_get($row, 2, '');
            $category = data_get($row, 3, '');
            $income = data_get($row, 4, '');
            $area_id = data_get($row, 5, '');
            $time = data_get($row, 6, '');
            $overtime = data_get($row, 7, '');
            $welfare = data_get($row, 8, '');
            $holiday = data_get($row, 9, '');
            $job_description = data_get($row, 10, '');
            $specific_content = data_get($row, 11, '');
            $work_environment = data_get($row, 12, '');
            $position_worthwhile = data_get($row, 13, '');
            $job_worthwhile = data_get($row, 14, '');
            $career_path = data_get($row, 15, '');
            $required_condition = data_get($row, 16, '');
            $welcome_condition = data_get($row, 17, '');
            $ideal_image = data_get($row, 18, '');
            $movie_title = data_get($row, 19, '');
            $movie = data_get($row, 20, '');
